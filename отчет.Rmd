 ...
 **X∼U(5,105)X∼U(5,105)  
 Y=f(X)+ϵY=f(X)+ϵ**  
 где f(x)= 8+3.5*cos(x-30)/12, ϵ∼N(0,1)ϵ∼N(0,1)
...
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 2 и равно 1.08. Что и является компромиссом между простотой и точностью модели.
...
Теперь изменим исходные данные, пусть количество наблюдений изменится с 60 до 600.
...
Сравнив полученные модели, можно увидеть, что при увеличении количества наблюдений, лучшая модель ближе к истинной функции, так же значение ошибки MSE на тестовой выборке уменьшается, в данном случае она изменилась с 1.08 до 1.07
